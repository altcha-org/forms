<script lang="ts">
  ...
  import { form } from '$lib/stores';
	...
	import type { IFormBlockPartial, IPdfInputOptions, IPdfInputElement } from '$lib/types';
  ...
  $: file = files?.[0];
  $: filteredBlocks = showAllFields ? blocks : blocks.filter(({ type }) => type && ['SignatureInput'].includes(type));
  ...
  async function loadPDFJS(src = '/vendor/pdfjs/pdf.min.mjs'): Promise<IPDFJS> {
    const exists = [...document.querySelectorAll('script')].find((el) => el.getAttribute('src') === src);
    if (!exists) {
  ...
  async function renderPage(num: number = 1) {
    if (doc && elViewport) {
      ...
      if (zoom === 1) {
        zoom = Math.max(0.4, Math.min(Math.min(1, elViewport.parentElement!.clientHeight / viewport.height), Math.min(1, elViewport.parentElement!.clientWidth / viewport.width)));
      }
  ...
  function onDragStart(ev: DragEvent, scale: number = 1) {
    ...
    ev.dataTransfer!.setData('text', new URLSearchParams({
  ...
  function onDrop(ev: DragEvent) {
    ...
    let el = params.get('id') ? elViewport.querySelector(`#${params.get('id')}`) as HTMLElement : null;
