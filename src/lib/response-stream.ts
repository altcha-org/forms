...
export class ResponseStream {
	...
	async stream() {
		...
		while (true) {
			...
			const hasMore =
				result.total > result.offset + result.responses.length &&
				result.responses.length === result.limit;
			if (!hasMore) {
				break;
			}
	...
	async decryptData(response: IResponse) {
		if (response.encrypted && response.dataEncrypted && response.encryptionKeyHash) {
			const data = await decryptData(
				response.dataEncrypted,
				response.encryptionKeyHash,
				get(encryptionKeys)
			);
			if (data === null) {
	...
	async fetchFiles(responseId: string) {
		const resp = await fetch(
			`/app/accounts/${this.options.accountId}/stream/responses/${responseId}/files`,
			{
				method: 'GET'
			}
		);
	...
	async fetchResponses(offset: number = 0, limit: number = 100): Promise<IResponseSearchResult> {
		const url = new URL(
			`/app/accounts/${this.options.accountId}/stream/responses`,
			location.origin
		);
