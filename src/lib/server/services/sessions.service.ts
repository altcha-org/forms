...
export class SessionsService {
    ...
    const correction = !data.abondoned && data.fields ? Math.floor(data.fields.reduce((acc, field) => {
			return acc + Math.max(0, field[3] - 1);
		}, 0) / data.fields.length * 100) : 0;
		await db
			.insert(sessions)
			.values({
				abondoned: data.abondoned,
        completionTime: !data.abondoned && data.submitAt && data.startAt ? data.submitAt?.getTime() - data.startAt.getTime() : null,
        correction,
	...
	async stats(options: ISessionStatsOptions) {
        ...
        mobile: sessions.mobile,
			...
			.from(sessions)
			.groupBy(sessions.abondoned, sessions.fieldDropOff, sessions.error, sessions.country, sessions.mobile, sql`date`)
			.where(
        and(
          eq(sessions.formId, options.formId),
					gte(sessions.id, idgen.prefixed(EIdPrefix.SESSION, idgen.boundary(options.dateStart.getTime(), false))),
					lte(sessions.id, idgen.prefixed(EIdPrefix.SESSION, idgen.boundary(options.dateEnd.getTime(), true))),
        )
      );
		...
		const labels: string[] = [];
		const correctionRate = Math.round(results.reduce((acc, { correctionRate }) => acc + (correctionRate as number), 0) / results.length) / 100;
		...
		const completionTime = results.reduce(
			(acc, { completionTime }) =>
				completionTime ? [...acc, completionTime as number] : acc,
			[] as number[]
		);
      ...
      countries: results.reduce((acc, { country, count }) => {
        if (!country) {
          country = 'unknown';
        }
      ...
      fieldDropOff: results.reduce((acc, { fieldDropOff, count }) => {
        if (fieldDropOff) {
