...
<Page title={$_('title.identities')}>
	<div class="prose max-w-full">
		<MarkdownRenderer value={$_('text.identities_info')} />
	</div>

	<div class="bg-base-200 p-4 rounded-lg">
		<div class="flex flex-col gap-3">
			<Form action="?/search" successToast={false} on:submit={(ev) => onSearchResult(ev.detail)}>
		...
		<div class="flex flex-col gap-3 mt-6">
			...
			{#each searchResults as identity}
				<div class="flex gap-4 border border-base-300 rounded-lg p-4">
					<div class="grow">
						<div class="font-bold">
							<a href="/app/accounts/{data.account.id}/identities/{identity.id}" class="link"
								>{identity.externalId}</a
							>
						</div>
						<div class="text-sm opacity-60">
							<span
								>{$_('label.created_ago', {
									values: { ago: formatTimeAgo(identity.createdAt) }
								})}</span
							>
							<span>&bull;</span>
