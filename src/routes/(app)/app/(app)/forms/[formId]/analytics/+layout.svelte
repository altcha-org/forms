<script lang="ts">
  ...
  const intervals = [
	...
	$: customDays = (parseISO(customEndDate).getTime() - parseISO(customStartDate).getTime()) / 86400000;
	...
	function getMatchingInterval(): typeof intervals[number] {
		for (const int of intervals) {
			...
			if (dates.end.getTime() === endDate.getTime() && dates.start.getTime() === startDate.getTime()) {
				return int;
			}
		}
		return 'custom';
		...
		<div class="text-sm flex gap-4 items-center justify-end">
			<div class="whitespace-nowrap opacity-60">
				...
				<button
					type="button"
					class="link ml-1"
					on:click|preventDefault={() => onIntervalChange()}
				>{$_('button.change')}</button>
				{/if}
				{/if}
			</div>
			...
			<label class="flex items-center gap-2">
				<input type="checkbox" class="checkbox checkbox-xs bg-base-100" bind:checked={$allResponses} />
	...
	<div class="flex flex-col gap-6">
		...
		<div class="flex flex-wrap gap-3 text-sm">
			...
			<button
				type="button"
				class="link"
				on:click|preventDefault={() => onPreset(preset)}
			>
				{preset.formatted}
			</button>
			{/each}
